...
export const removeToken = async (token: string) => {
  return await TokenDbModel.findOneAndDelete({ refreshToken: token });
};
...
export const cookieTokens = (token: string, refreshToken: string, res: Response) => {
...
export const createSendToken = async (user: User, statusCode: number, res: Response) => {
